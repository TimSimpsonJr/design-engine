...
> **Active configuration:** Check `.design-rules/config.json` in the current project for the active adapter, skin, and recipe. This skill applies the 69 mobile-dashboard rules below to the active context. If no config exists, the rules still apply universally — suggest `/design-init` to capture project-specific decisions.
>
> **Stack mapping:** This skill uses Tailwind v4 utility classes (e.g., `text-[36px]`, `mx-6`, `bg-card`) as the canonical examples because the original styleseed engine targets Tailwind v4. For non-Tailwind adapters:
> - **Plain CSS / Obsidian / vanilla**: translate `text-[36px]` to `font-size: 36px`, `mx-6` to `margin-left: 24px; margin-right: 24px`, etc. Token names (`--brand`, `--card`, `--foreground`) are the same regardless of stack.
> - **Astro / SvelteKit with Tailwind v4**: classes apply directly.
> - **react-shadcn**: classes apply directly + you have access to ported pattern components (HeroCard, ChartCard, etc.) — see `design-engine` skill's pattern catalog.
>
> **Composition recipes** (rules 61-69) live in this skill but also load via the `composition-recipes` skill when scaffolding pages.
